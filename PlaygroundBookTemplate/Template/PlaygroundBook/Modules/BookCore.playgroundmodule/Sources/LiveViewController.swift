...
extension LiveViewController: ARCoachingOverlayViewDelegate {
...
extension InfectionHandler {
    func checkIfDone() -> Bool{
        ...
        let susceptibleNodes = graph.nodes
            .filter({$0.SIRState == .susceptible})
            .filter({$0.metaData != .vaccinated && $0.metaData != .quarantined})
        let edgesForSusNodes = susceptibleNodes.flatMap({$0.edges})
