...
// RED EDGES?
//            guard let graphnode = graph.nodes.first(where: { $0.toRKFormat() == nodeEntity.name}) else { return }
//            for node in graph.nodes {
//                for edge in node.edges {
//                    if edge.u.id == graphnode.id || edge.v.id == graphnode.id {
//                        if let e = anchorEntity.findEntity(named: edge.toRKFormat()) as? ModelEntity {
//                            e.model?.materials = [SimpleMaterial(color: .red, isMetallic: false)]
//                        }
//                        if let e = anchorEntity.findEntity(named:  edge.reverseRKFormat()) as? ModelEntity {
//                            e.model?.materials = [SimpleMaterial(color: .red, isMetallic: false)]
//                        }
//                    }
//                }
